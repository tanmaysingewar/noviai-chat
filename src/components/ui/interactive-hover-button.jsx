...
const InteractiveHoverButton = React.forwardRef(({ text = "Button", className, ...props }, ref) => {
  return (
    (<button
      ref={ref}
      className={cn(
        "group relative w-32 cursor-pointer overflow-hidden rounded-full bg-background p-2 text-center font-semibold group-hover:bg-purple-800",
        className
      )}
      {...props}>
      <span
      ...
      <div
        className="absolute left-[20%] top-[40%] h-2 w-2 scale-[1] rounded-lg bg-primary bg-violet-900 transition-all duration-300 group-hover:left-[0%] group-hover:top-[0%] group-hover:h-full group-hover:w-full group-hover:scale-[1.8] group-hover:bg-purple-900 group-hover:bg-opacity-90"></div>
    </button>)
  );
});
